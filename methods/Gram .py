...
def callGram(y,u,ny,nu,l,limit):
    Psi = matrix_candidate(u,y,ny,nu,l)
    PsiSym = symbolic_candidates(nu,ny,l)
...
def gramPlot(y_h,y_h2,u_h,u_h2,lim,ny,nu,l):
    ...
    print(theta,'\n')
    Psi1 = matrix_candidate(u_h,y_h,ny,nu,l)
    p1 = ortogonalize(Psi1,r)
    Psi2 = matrix_candidate(u_h2,y_h2,ny,nu,l)
...
if __name__ == '__main__':
    ...
    filenames = ["ballbeam.dat","exchanger.dat","robot_arm.dat","SNLS80mV.dat","Schroeder80mV.dat"]
    lims = [4,3,4,7,4,5]
    for i in range(6):
        f = filenames[i]
        lim = lims[i]
        ### Pasta onde tão os arquivos
        filename = "data/" + f
        doIdentification(filename,lim,ny,nu,l)
    #lim = int(input("Inform the size limit\n"))
    #doIdentification(filename,lim,ny,nu,l)
    """MELHOR LIM PRA CADA CASO:"""
    """BALLBEAM: 4
       DATABENCHMARK: 3-4
       EXCHANGER: 4-5
       ROBOTARM: 7
